...
public:
    ...
    float amplitude;
    float pitch;
    float loopStartFraction;
    float loopEndFraction;
    float loopCrossFadeFraction;
    float attack;
    float decay;
	bool loop;

	cSoundSettings( void ){ startFraction = 0; endFraction = 1.0; loopStartFraction = 0.1; loopEndFraction = 0.9; pitch = 1.0; amplitude = 1.0; loop = false; loopCrossFadeFraction = .1; attack = .1; decay = .1; }
    void Init( float start, float end, float amp, float pitch_, bool _loop, float _loopStart, float _loopEnd, float _loopCrossFadeFraction, float _attack, float _decay ) { startFraction = start; endFraction = end; amplitude = amp; pitch = pitch_; loop = _loop; loopStartFraction = _loopStart; loopEndFraction = _loopEnd; loopCrossFadeFraction = _loopCrossFadeFraction; attack = _attack; decay = _decay; }
...
public:
    ...
    cSoundDefinition();
    ~cSoundDefinition();
    void Init( ofxAudioFile& file, cSoundSettings* settings );
    bool LoadSample( string samplePath, bool init = false );
    ...
    float ApplyEnvelope( ofSoundBuffer& buffer, double position, float inEnvelope, T_EnvelopeMode *envelopeMode, float currentLevel = 1.0, float slewRate = .0001, float targetLevel = 1.0 );
